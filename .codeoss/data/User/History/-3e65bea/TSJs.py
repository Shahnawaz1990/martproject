...
@app.post("/orders/{product_id}", response_model=Order)
def create_order(order: Order, session: Annotated[Session, Depends(get_session)], product_id: int):
        product = session.exec(select(Product).where(Product.id == product_id)).first()
        print(product)
        # order.product = product.dict()
        session.add(order)
        session.commit()
        session.refresh(order)
        return order

@app.put("/orders/{order_id}/{product_id}", response_model=Order)
def update_product_in_order_by_id(order: Order, order_id: int, product_id: int, session: Annotated[Session, Depends(get_session)]):
    orderbyid = session.exec(select(Order).where(Order.id == order_id)).first()
    product = session.exec(select(Product).where(Product.id == product_id)).first()
    # print(f"printing orderbyid.product {orderbyid.product}")
    # print(f"printing orderbyid.product {type(orderbyid.product)}")
    # print(f"printing product {product.dict()}")
    # print(f"printing product {type(product.dict())}")
    orderbyid.product.update(product.dict())
    for i, v in orderbyid.product.items():
        print(f"item printing: {i} : {v}")
        session.add(i)
        session.add(v)
        session.commit()
    return orderbyid
...
def update_order_by_id(order: Order, order_id: int, session: Annotated[Session, Depends(get_session)]):
