...
def create_payment(payment: Payment, session: Annotated[Session, Depends(get_session)], order_id: int):
        ...
        # Set up the MIME message
            ...
            msg_str = json.dumps(payment.dict())
            msg.attach(MIMEText(msg_str, 'plain'))

        # Send the email via SMTP
            with smtplib.SMTP("smtp.gmail.com", 587) as server:
                server.starttls()  # Secure the connection
                server.login("shahnawaz.nazimali@gmail.com", "lusc tqxv vuvl pgse")
                server.sendmail("shahnawaz.nazimali@gmail.com", payment.email, msg.as_string())
                print(f"Email sent to {payment.email}")
        except Exception as e:
            print(f"Failed to send email: {str(e)}")
            raise HTTPException(status_code=500, detail="Failed to send email")
        return (f"Email sent to {payment.email}")