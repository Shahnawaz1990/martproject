...
@app.get("/purchases/qty/{product_id}")
def sumofqty_by_productid(session: Annotated[Session, Depends(get_session)], product_id: int):
        purchases = session.exec(select(Purchase)).all()
        p = []
        for i in purchases:
            for j, k in i.product.items():
        ...
        sumqty = 0;
        for q in p:
            print(f"The qty of {q.product.content} is {q.qty}")
            sumqty += q.qty

        print(f"the sum of {q.product.content} is {sumqty}")
